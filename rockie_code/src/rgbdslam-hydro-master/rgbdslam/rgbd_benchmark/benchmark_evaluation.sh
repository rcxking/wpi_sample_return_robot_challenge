...
for CANDIDATES in  2 4 6; do
  ...
  for OBS_EVAL in  0.8 0.9 0; do
    rosparam set /rgbdslam/config/observability_threshold $OBS_EVAL
    for RANSAC_ITER in 501; do
      rosparam set /rgbdslam/config/ransac_iterations $RANSAC_ITER

      for SOLVER in csparse; do 
        rosparam set /rgbdslam/config/backend_solver $SOLVER

        for NN_RATIO in 0.95; do
          ...
          for MIN_MATCHES in 020; do
            ...
            for FEAT_TYPE in GFTT SIFTGPU ORB SURF ; do 
              ...
              for MAXFEATURES in 0600; do
                rosparam set /rgbdslam/config/max_keypoints $MAXFEATURES
                PARAM_DIRECTORY="$BASE_DIRECTORY/$3/emm__$OBS_EVAL/CANDIDATES_$CANDIDATES/RANSAC_$RANSAC_ITER/SOLVER_$SOLVER/NN_$NN_RATIO/MIN_MATCHES_$MIN_MATCHES/${FEAT_TYPE}/${MAXFEATURES}_Features/"
