        ...
        for( int x = 0; x < w-step; x+=step){//for it and its neighbours right and/or down
            ...
            if(!strip_on){ //Generate vertices for new triangle
                ...
                if(validXYZ(*ul)){ //ul, ur, ll all valid
                  depth = glv_squaredEuclideanDistance(*ul,origin);
                  if (glv_squaredEuclideanDistance(*ul,*ll)/depth <= mesh_thresh  and 
                      glv_squaredEuclideanDistance(*ul,*ll)/depth <= mesh_thresh  and
                      glv_squaredEuclideanDistance(*ur,*ll)/depth <= mesh_thresh){
                ...
                if(!strip_on) { //can't use the point on the upper left, should I still init a triangle?
                  ...
                  if(!validXYZ(*lr)) {
                    ...
                    depth = glv_squaredEuclideanDistance(*ur,origin);
                    if (glv_squaredEuclideanDistance(*ur,*ll)/depth <= mesh_thresh  and 
                        glv_squaredEuclideanDistance(*lr,*ll)/depth <= mesh_thresh  and
                        glv_squaredEuclideanDistance(*ur,*lr)/depth <= mesh_thresh){
                      glBegin(GL_TRIANGLE_STRIP);
                      strip_on = true;
            ...
            {//neighbours to the left and left down are already set
              ...
              if(validXYZ(*ul)){ //Neighbours to the left are prepared
                depth = glv_squaredEuclideanDistance(*ul,origin);
                if (glv_squaredEuclideanDistance(*ul,*(ul-step))/depth > mesh_thresh){
                  glEnd();
                  strip_on = false;
                  continue;
                }
              ...
              if(validXYZ(*ll)){ 
                depth = glv_squaredEuclideanDistance(*ll,origin);
                if (glv_squaredEuclideanDistance(*ul,*ll)/depth > mesh_thresh or
                    glv_squaredEuclideanDistance(*ul,*(ul-step))/depth > mesh_thresh or
                    glv_squaredEuclideanDistance(*ll,*(ll-step))/depth > mesh_thresh){
                  glEnd();
                  strip_on = false;
                  continue;
                }
...
void GLViewer::pointCloud2GLList(pointcloud_type const * pc){
    ...
    for(unsigned int x = 0; x < w-1; x++){
        for(unsigned int y = 0; y < h-1; y++){
            ...
            depth = glv_squaredEuclideanDistance(*pi,origin);

            const point_type* pl = &pc->points[(x+1)+(y+1)*w]; //one right-down
            if(!(validXYZ(*pl)) or glv_squaredEuclideanDistance(*pi,*pl)/depth > mesh_thresh)  
              continue;

            const point_type* pj = &pc->points[(x+1)+y*w]; //one right
            if(validXYZ(*pj)
               and glv_squaredEuclideanDistance(*pi,*pj)/depth <= mesh_thresh  
               and glv_squaredEuclideanDistance(*pj,*pl)/depth <= mesh_thresh){
              drawTriangle(*pi, *pj, *pl);
            }
            const point_type* pk = &pc->points[(x)+(y+1)*w]; //one down
            
            if(validXYZ(*pk)
               and glv_squaredEuclideanDistance(*pi,*pk)/depth <= mesh_thresh  
               and glv_squaredEuclideanDistance(*pk,*pl)/depth <= mesh_thresh){
